...
exports.parse = function(str, line, parser, types) {
  ...
  parser.on(types.ATTR_VAL, function(token) {
    var matches = token.match.match(/^([a-zA-Z]+)="([^"]+?)"/);

    if ('expr' !== matches[1]) {
      throw new Error('Unexpected attribute "' + matches[1] + '" on line ' + line + '.');
    }

    this.out.push(matches[2]);
